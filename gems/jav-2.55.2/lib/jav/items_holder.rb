module Jav
  class ItemsHolder
    attr_reader :tools
    attr_accessor :items, :invalid_fields
    ...
    def field(field_name, **args, &block)
      field_parser = Jav::Dsl::FieldParser.new(id: field_name, order_index: @items_index, **args, &block).parse
    ...
    def tab(name, **args, &block)
      add_item Jav::TabBuilder.parse_block(name: name, **args, &block)
    end

    def row(**args, &block)
      add_item Jav::RowBuilder.parse_block(**args, &block)
    end
    ...
    def panel(panel_name = nil, **args, &block)
      panel = Jav::PanelBuilder.parse_block(name: panel_name, **args, &block)

      add_item panel
    end

    def heading(body = nil, **args, &block)
      field = Jav::Fields::HeadingField.new(body, **args)
