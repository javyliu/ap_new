  ...
  module Concerns
    module FiltersSessionHandler
      ...
      def filters_from_params
        params[Jav::Filters::BaseFilter::PARAM_KEY].presence
      end

      def save_filters_to_session
        session[filters_session_key] = params[Jav::Filters::BaseFilter::PARAM_KEY]
      end
      ...
      def cache_resource_filters?
        return Jav.configuration.cache_resource_filters unless Jav.configuration.cache_resource_filters.is_a?(Proc)

        Jav.configuration.cache_resource_filters.call(current_user: current_user, resource: @resource)
      end
    end
  end
end
