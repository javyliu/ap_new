...
class Jav::TabSwitcherComponent < Jav::BaseComponent
  include Jav::UrlHelpers
  include Jav::ApplicationHelper

  attr_reader :active_tab_name, :group, :current_tab, :tabs, :view, :style
  ...
  def single_item_visible?(item)
    ...
    return true unless item.is_field?
    return true unless item.has_own_panel?
