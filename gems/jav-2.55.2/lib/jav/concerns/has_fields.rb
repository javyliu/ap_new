    ...
    module HasFields
      ...
      class_methods do
        ...
        def tabs(**args, &block)
          ...
          items_holder.tabs Jav::TabGroupBuilder.parse_block(**args, &block)
        end
        ...
        def sidebar(**args, &block)
          ...
          items_holder.sidebar Jav::SidebarBuilder.parse_block(**args, &block)
        end
        # END DSL methods
        ...
        def fields(only_root: false)
          ...
          items.each do |item|
            ...
            fields << extract_fields_from_items(tab) if item.is_row?
          end
        ...
        def ensure_items_holder_initialized
          self.items_holder ||= Jav::ItemsHolder.new
        end
      ...
      def check_license
        return unless !Rails.env.production? && App.license.present? && App.license.lacks(:resource_tools)

        # Add error message to let the developer know the resource tool will not be available in a production environment.
        Jav::App.error_messages.push "Warning: Your license is invalid or doesn't support resource tools. The resource tools will not be visible in a production environment."
      end
    end
  end
end
