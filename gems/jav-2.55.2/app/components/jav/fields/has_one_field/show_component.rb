...
class Jav::Fields::HasOneField::ShowComponent < Jav::Fields::ShowComponent
  ...
  def can_attach?
    ...
    if @field.present?
      ...
      if reflection_resource.present? && @resource.present?
        ...
        policy_result = @resource.authorization.authorize_action(method_name, raise_exception: false) if @resource.authorization.has_method?(method_name, raise_exception: false)
      end
    end
  ...
  def attach_path
    helpers.jav.resources_associations_new_path(@resource.singular_model_key, @resource.model.to_param, @field.id)
  end
