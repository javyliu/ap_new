...
module Jav
  class SearchController < ApplicationController
    ...
    def index
      resources = Jav::App.resources.reject do |resource|
    ...
    def search_resource(resource)
      query = Jav::Hosts::SearchScopeHost.new(
      ...
      header = "#{header} (#{results_count})" if results_count.positive?
    ...
    def apply_attach_scope(query, parent)
      ...
      Jav::Hosts::AssociationScopeHost.new(block: attach_scope, query: query, parent: parent).handle
    end
    ...
    def apply_has_many_scope
      ...
      Jav::Hosts::SearchScopeHost.new(block: @resource.search_query, params: params, scope: scope).handle
    end

    def apply_search_metadata(models, jav_resource)
      models.map do |model|
        resource = jav_resource.dup.hydrate(model: model).hydrate_fields(model: model)

        record_path = if resource.search_result_path.present?
                        Jav::Hosts::ResourceRecordHost.new(block: resource.search_result_path, resource: resource, record: model).handle
                      else
    ...
    def fetch_field
      fields = ::Jav::App.get_resource_by_model_name(params[:via_reflection_class]).get_field_definitions
      fields.find { |f| f.id.to_s == params[:via_association_id] }
    end

    def fetch_parent
      return if params[:via_reflection_id].blank?

      parent_resource = ::Jav::App.get_resource_by_model_name params[:via_reflection_class]
