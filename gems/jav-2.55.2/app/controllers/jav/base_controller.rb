...
module Jav
  class BaseController < ApplicationController
    include Jav::Concerns::FiltersSessionHandler

    before_action :set_resource_name
    before_action :set_resource
    before_action :hydrate_resource
    before_action :set_applied_filters, only: :index
    before_action :set_model, only: %i[show edit destroy update order]
    ...
    def cast_nullable(params)
      ...
      nullable_fields = fields.filter(&:nullable).to_h do |field|
      ...
      params.each do |key, value|
        ...
        params[key] = nil if nullable_values.present? && value.in?(nullable_values)
      end
    ...
    def set_index_params
      ...
      @index_params[:per_page] = Jav.configuration.per_page

      @index_params[:per_page] = cookies[:per_page] if cookies[:per_page].present?

      @index_params[:per_page] = Jav.configuration.via_per_page if @parent_model.present?
      ...
      # View types
      @index_params[:view_type] = params[:view_type] || @resource.default_view_type || Jav.configuration.default_view_type
      @index_params[:available_view_types] = @resource.available_view_types
    end

    def set_filters
      @filters = @resource.get_filters.map do |filter|
    ...
    def set_applied_filters
      ...
      @applied_filters = Jav::Filters::BaseFilter.decode_filters(fetch_filters)
    ...
    def reactive_filters
      ...
      @resource.get_filters.select do |filter|
    ...
    def filters_to_be_applied
      ...
      @resource.get_filters.each do |filter|
        ...
        filter_defaults[filter.class.to_s] = filter.default unless filter.default.nil?
      end
    ...
    def set_edit_title_and_breadcrumbs
      ...
      if params[:via_resource_class].present? && params[:via_resource_id].present?
        ...
        add_breadcrumb via_resource.plural_name, resources_path(resource: via_resource)
        add_breadcrumb via_resource.model_title, resource_path(model: via_model, resource: via_resource, active_tab_name: params[:active_tab_name])
    ...
    def create_success_message
      "#{@resource.name} #{t('jav.was_successfully_created')}."
    end
    ...
    def after_create_path
      ...
      if is_associated_record?
        parent_resource = ::Jav::App.get_resource_by_model_name(params[:via_relation_class]).dup
        association_name = BaseResource.valid_association_name(@model, params[:via_relation])
    ...
    def update_success_message
      "#{@resource.name} #{t('jav.was_successfully_updated')}."
    end
    ...
    def redirect_path_from_resource_option(action = :after_update_path)
      ...
      elsif @resource.class.send(action) == :edit || Jav.configuration.resource_default_view == :edit
    ...
    def set_pagy_locale
      @pagy_locale = locale.to_s || Jav.configuration.locale || "en"
    end
