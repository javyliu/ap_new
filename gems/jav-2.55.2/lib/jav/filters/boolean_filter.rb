module Jav
  module Filters
    class BooleanFilter < BaseFilter
      self.template = "jav/base/boolean_filter"
      ...
      def selected_value(item, applied_filters)
        ...
        if applied_filters[self.class.to_s].present?
          ...
          applied_filters[self.class.to_s].stringify_keys[item.to_s]
        else
          applied_or_default_value(applied_filters).stringify_keys[item.to_s]
        end
