...
export default class extends Controller {
  ...
  toggle({ params }) {
    ...
    if (toggleTarget) {
      this.toggleJavTarget(toggleTarget)
    }

    if (toggleTargets && toggleTargets.length > 0) {
      toggleTargets.forEach(this.toggleJavTarget.bind(this))
    }
  }
  ...
  disable({ params }) {
    ...
    if (disableTarget) {
      this.disableJavTarget(disableTarget)
    }

    if (disableTargets && disableTargets.length > 0) {
      disableTargets.forEach(this.disableJavTarget.bind(this))
    }
  }

  // Private

  toggleJavTarget(targetName) {
