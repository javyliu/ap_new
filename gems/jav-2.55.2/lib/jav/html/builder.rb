class Jav::HTML::Builder
  ...
  attr_accessor :wrapper_stack, :data_stack, :style_stack, :classes_stack, :show_stack, :edit_stack, :index_stack, :input_stack, :label_stack, :content_stack, :record, :resource
  ...
  def assign_property(property = :data, payload = nil, &block)
    value = if block.present?
              Jav::Hosts::RecordHost.new(block: block, record: record).handle
            else
              payload
            end
