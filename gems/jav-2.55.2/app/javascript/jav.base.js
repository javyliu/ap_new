...
import 'chart.js/auto'  // 替代原来的 chartkick/chart.js/chart.esm
...
document.addEventListener('turbo:before-fetch-response', async (e) => {
  if (e.detail.fetchResponse.response.status === 500) {
    ...
    if (!e.detail.fetchResponse?.response?.url?.includes('/failed_to_load')) {
      e.target.src = `${window.Jav.configuration.root_path}/failed_to_load?turbo_frame=${id}&src=${src}`
    }
  }
})
...
window.Jav = window.Jav || { configuration: {} }
