module Jav
  class BaseCard
    ...
    attr_accessor :dashboard, :options, :arguments, :index, :params
    ...
    def initialize(dashboard:, options: {}, arguments: {}, index: 0, cols: nil, rows: nil, label: nil,
                   description: nil, refresh_every: nil, visible: nil)
      @dashboard = dashboard
      @options = options
      @arguments = arguments
      @index = index
    ...
    def frame_url(enforced_range: nil, params: {})
      ...
      Jav::App.view_context.jav.dashboard_card_path(dashboard.id, id, turbo_frame: turbo_frame, index: index,
                                                                      range: enforced_range, **params.permit!)
    end
    ...
    def type
      return :metric if is_a?(::Jav::Dashboards::MetricCard)
      return :chartkick if is_a?(::Jav::Dashboards::ChartkickCard)

      :partial if is_a?(::Jav::Dashboards::PartialCard)
    end

    def compute_result
      Jav::Hosts::DashboardCard.new(card: self, dashboard: dashboard, params: params, context: context, range: range,
                                    options: options)
                               .compute_result
