module Jav
  module UrlHelpers
    def resources_path(resource:, keep_query_params: false, **args)
      ...
      jav.send :"resources_#{route_key}_path", **existing_params, **args
    end
    ...
    def resource_path(
      ...
      **args
      ...
      jav.send :"resources_#{resource.singular_route_key}_path", id, **args
    end

    def new_resource_path(resource:, **args)
      jav.send :"new_resources_#{resource.singular_route_key}_path", **args
    end

    def edit_resource_path(model:, resource:, **args)
      jav.send :"edit_resources_#{resource.singular_route_key}_path", model, **args
    end

    def resource_attach_path(resource, model_id, related_name, related_id = nil)
      helpers.jav.resources_associations_new_path(resource.singular_route_key, model_id, related_name)
    end
    ...
    def resource_detach_path(
      ...
      jav.resources_associations_destroy_path(model_name, model_id, related_name, related_id)
    end
    ...
    def related_resources_path(
      ...
      **args
      ...
      jav.resources_associations_index_path(parent_model.model_name.route_key, record.id, **existing_params, **args)
    end

    def resource_view_path(**args)
      if Jav.configuration.resource_default_view == :edit
